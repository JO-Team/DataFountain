...
train = pd.read_csv(
    '../input/user_action_train.txt', sep="\t", header=None, parse_dates=[3])
goods = pd.read_csv('../input/goods_train.txt', sep="\t", header=None)
test = pd.read_csv(
    '../input/user_action_test_items.txt.', sep="\t", header=None)
...
train.rename(
    columns={0: 'uid',
             ...
             3: 'date'}, inplace=True)
goods.rename(
    columns={0: 'spu_id_goods',
             1: 'brand_id',
             .2: 'cat_id'}, inplace=True)
test.rename(columns={0: 'uid', 1: 'spu_id'}, inplace=True)

train_goods = pd.merge(
    train, goods, left_on='spu_id', right_on='spu_id_goods', how='left')
train_goods.rename(columns={5: 'cat_id'}, inplace=True)

train_label = train['action_type']
train_goods = train_goods.drop(
    ['action_type', 'date', 'spu_id_goods', 'brand_id'], axis=1)
